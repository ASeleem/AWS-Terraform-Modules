locals {
  # Get distinct list of domain names
...
resource "aws_route53_record" "validation" {
  count                       = (var.create_certificate || var.create_route53_records_only) && var.validation_method == "DNS" && var.create_route53_records && (var.validate_certificate || var.create_route53_records_only) ? length(local.distinct_domain_names) : 0
...
resource "aws_acm_certificate_validation" "this" {
  count                       = var.create_certificate && var.validation_method != null && var.validate_certificate && var.wait_for_validation ? 1 : 0
